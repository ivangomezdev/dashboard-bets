...
const RESULT_OVERRIDES = new Map([
  ...
  [
    "1786645433271-pvcpkg",
    {
      ...
      profitUsd: 5.65,
      payoutMxn: 948.94,
      reason: "ajuste de cierre para reconciliar la ganancia diaria confirmada de 7 USD"
    }
  ]
]);

const LEG_BOOKER_OVERRIDES = new Map([
  [
    "1786673671841-sya8lq",
    {
      legIndex: 1,
      booker: "BetOnline",
      vps: "VPS 2",
...
function normalizeArb(row, timezone) {
  ...
  const legBookerOverride = LEG_BOOKER_OVERRIDES.get(row.id);
  ...
  const compactLegs = (Array.isArray(row.legs) ? row.legs.map((leg) => compactLeg(leg, row)) : []).map(
    (leg) =>
      legBookerOverride && leg.index === legBookerOverride.legIndex
        ? {
            ...leg,
            booker: legBookerOverride.booker,
            bookerBase: legBookerOverride.booker,
            bookerLabel: bookerLabel(legBookerOverride.booker, legBookerOverride.vps),
            bookerKey: bookerLabel(legBookerOverride.booker, legBookerOverride.vps),
            vps: legBookerOverride.vps,
            currency: legBookerOverride.currency
          }
        : leg
  );
  ...
  return {
    ...
    reason: resultOverride?.reason || legBookerOverride?.reason || row.reason || "",
