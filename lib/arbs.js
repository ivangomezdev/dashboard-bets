...
function compactLeg(leg) {
  return {
    index: leg.index,
    booker: leg.displayBooker || leg.booker || "Sin booker",
    ...
    outcome: leg.settlementOutcome || leg.outcome || null,
...
function normalizeArb(row, timezone) {
  ...
  const profitMxn = numberOrZero(row.actualProfitMxn);
  ...
  return {
    ...
    closedAt,
    dateKey: closedAt ? localDateKey(closedAt, timezone) : "Sin fecha",
    reason: row.reason || "",
    totalStakeMxn: numberOrZero(row.totalStakeMxn),
    payoutMxn: numberOrZero(row.payoutMxn),
    profitMxn,
    profitUsd: numberOrZero(row.actualProfitUsd),
    projectedProfitMxn: numberOrZero(row.projectedProfitMxn),
    winningBooker: row.winningLegBooker || "Sin ganador",
    status: profitMxn >= 0 ? "profit" : "loss",
    legs: Array.isArray(row.legs) ? row.legs.map(compactLeg) : []
  };
}
...
function summarizeBookers(arbs) {
  ...
  for (const arb of arbs) {
    for (const leg of arb.legs) {
      const current = bookers.get(leg.booker) || {
        name: leg.booker,
        count: 0,
        won: 0,
        lost: 0,
      ...
      current.stake += leg.currency === "MXN" ? leg.stake : 0;
      bookers.set(leg.booker, current);
    }
  }
