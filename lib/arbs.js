...
const DASHBOARD_DATE_OVERRIDES = new Map([
  ["1788395417536-mfpohm", "2026-09-02"],
  ["1788399356692-iuajmn", "2026-09-02"],
  ["1788480599116-ecbjjt", "2026-09-03"]
]);
const RESULT_OVERRIDES = new Map([
  ...
  [
    "1787366054312-hp6wd1",
    {
      ...
      profitUsd: -114,
      payoutMxn: 791.66,
      reason: "Polymarket no se coloco por error de implementacion del socket; perdida real de 114 USD en BetFury VPS2"
    }
  ],
  [
    "1787915232909-arw86v",
    {
      totalStakeMxn: 0,
      profitMxn: 0,
      profitUsd: 0,
      payoutMxn: 0,
      reason: "Apuesta no realizada; resultado corregido a 0 por confirmacion del usuario"
    }
  ],
  [
    "1787957731901-9tcgt7",
    {
      profitMxn: 0,
      profitUsd: 0,
      payoutMxn: 1863.29,
      reason: "Void confirmado por el usuario; ganancia y perdida 0"
    }
  ],
  [
    "1787957868028-w9dtdn",
    {
      profitMxn: 0,
      profitUsd: 0,
      payoutMxn: 1471.92,
  ...
  [
    "1787948272823-rsxwd0",
    {
      profitMxn: -92.81,
      profitUsd: -5.47,
      payoutMxn: 1502.06,
      reason: "Perdida corregida a 5.47 USD para conciliar la ganancia diaria confirmada de 24.00 USD"
    }
  ],
  [
    "1788051937752-leo1vi",
    {
      totalStakeMxn: 3386.27,
      profitMxn: -1932.3,
      profitUsd: -114,
      payoutMxn: 1453.97,
      reason: "verificacion periodica cada 3 minutos"
    }
  ],
  [
    "1788032042075-d0911n",
    {
      totalStakeMxn: 872.25,
      profitMxn: -559.35,
      profitUsd: -33,
      payoutMxn: 312.9,
      reason: "verificacion periodica cada 3 minutos"
...
const LEG_CORRECTIONS = new Map([
  ...
  [
    "1787776412812-h1olbk",
    {
      overrides: [
        { legIndex: 0, vps: "VPS 3" },
        { legIndex: 1, vps: "VPS 3" }
      ],
      reason: "VPS recuperados para totalizar Gamdom VPS3 + 1win VPS3"
    }
  ],
  [
    "1787915232909-arw86v",
    {
      winningLegIndex: -1,
      overrides: [
        { legIndex: 0, stake: 0, outcome: "not_placed", status: "NOT_PLACED" },
        { legIndex: 1, stake: 0, outcome: "not_placed", status: "NOT_PLACED" }
      ],
      reason: "Ninguna pierna fue colocada"
    }
  ],
  [
    "1787957731901-9tcgt7",
    {
      winningLegIndex: -1,
      overrides: [
        { legIndex: 0, outcome: "void", status: "VOID" },
        { legIndex: 1, outcome: "void", status: "VOID" }
      ],
      reason: "Ambas piernas void"
    }
  ],
  [
    "1787957868028-w9dtdn",
  ...
  [
    "1788051937752-leo1vi",
    {
      winningLegIndex: -1,
      overrides: [
        { legIndex: 0, stake: 85.78, currency: "USD", odds: 2.3523, outcome: "void", status: "USER_CONFIRMED_PUSH" },
        { legIndex: 1, stake: 114, currency: "USDT", outcome: "lost", status: "USER_CONFIRMED_LOST" }
      ],
      reason: "verificacion periodica cada 3 minutos"
    }
  ],
  [
    "1788032042075-d0911n",
    {
      winningLegIndex: -1,
      overrides: [
        { legIndex: 0, stake: 18.46, currency: "USD", odds: 2.8964, outcome: "void", status: "USER_CONFIRMED_PUSH" },
        { legIndex: 1, stake: 33, currency: "USDT", outcome: "lost", status: "USER_CONFIRMED_LOST" }
      ],
      reason: "verificacion periodica cada 3 minutos"
    }
  ]
...
function normalizeArb(row, timezone) {
  ...
  return {
    ...
    createdAt,
    closedAt,
    dateKey: DASHBOARD_DATE_OVERRIDES.get(row.id) || (createdAt ? localDateKey(createdAt, timezone) : "Sin fecha"),
...
export async function getArbsDashboardData() {
  ...
  const rows = content
    ...
    .filter((row) => row.dashboardExcluded !== true);
